class Pengguna {
    ...
    createElement() {
        // MAKE A NEW DIV WITH PROFILE CLASS
    ...
    getUser() {
        // SAME AS PROFILE WITHOUT EDIT BUTTON
        rdb.ref("users").child(this.uid).once("value", (data) => {
            ...
            this.element.querySelector(".bio").innerText = snap.bio?`“${snap.bio}”`:"No Bio";
    ...
    getFollower(uid) {
        ...
        rdb.ref("users/" + uid + "/follower").once("value", data => {
            if(data.exists()) { // KALO ADA KEY DI DALAMNYA
                ...
                this.element.querySelector(".followers").innerHTML = `${this.selang.pengguna.followers}:${data.numChildren()}`;
            } else { // KALO GADA
                // KASIH STRING 0
                this.element.querySelector(".followers").innerHTML = `${this.selang.pengguna.followers}:0`;
            }
        })
    }
    ...
    openChat(uid, nama, foto) {
        //CLOSE THIS ELEMENT
        this.element.remove();
        // RUN THE MESSAGE INSTANCE
        this.pesan = new Pesan({
            uid: uid,
            nama: nama,
            foto: foto
        });
        // INSERT CONTAINER INTO INIT -- PARAMETERS --

    ...
    getUserData() {
        ...
        rdb.ref("users/" + this.uid + "/follower").once("value", data => {
            if(data.exists()) { // KALO DI DALAMNYA TERDAPAT KEY
                ...
                this.element.querySelector(".followers").innerHTML = `${this.selang.profil.followers}:${data.numChildren()}`;
            } else { // KALO GADA
                // TAMPILKAN STRING 0
                this.element.querySelector(".followers").innerHTML = `${this.selang.profil.followers}:0`;
            }
        })
    }

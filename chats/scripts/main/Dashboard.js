...
var folder = path.substring(0, path.lastIndexOf('chats'));
...
class Dashboard {
    ...
    createElement() {
        ...
        `);
        // MAKE NEW DIV WITH CLASS NAVBAR
        ...
        this.navbar.innerHTML = (`
            ...
            <button class="bar-1 home"><i class="fa-solid fa-house"></i> ${this.selang.dashboard.navbarHome}</button>
            <button class="bar-1 Add"><i class="fa-solid fa-plus"></i> ${this.selang.dashboard.navbarAddItem}</button>
        `);
    ...
    detectLastActivity() {
        // LAST ACTIVITY DETECTION TO IMMEDIATELY OPEN THE ACTIVITY
        try {
            // CHECK WHETHER THE LAST ACTIVITY IS EXISTING AND THE FILLINGS ARE SAFE
    ...
    options() {
        ...
        this.container.querySelector(".navbar .home").onclick = () =>  window.location.href =  folder + "TradinGo.html";
        this.container.querySelector(".navbar .Add").onclick = () =>  window.location.href = folder + "admin.html";
    }
    ...
    async init() {
        ...
        */
        ...
        await userRef.once("value", (key) => {
            // UPDATE SOME SPECIAL DATA RELATED TO THE PROVIDER ONLY
            userRef.update({
